...
/* ── Scroll-observer: marks stagger items visible when they enter the viewport ── */
...
/* ── Reusable stagger grid (sets up IntersectionObserver) ── */
    ...
    <div className="home-shell">
          ...
          <div className="mt-12 grid gap-px overflow-hidden rounded-[20px] border border-[color:var(--dxy-border)] bg-[color:var(--dxy-border)] sm:grid-cols-2 lg:grid-cols-5">
            {heroMetrics.map((metric, i) => (
              <div
                key={metric}
                className={
                  "bg-[color:var(--dxy-card)] px-5 py-5 text-center text-sm font-medium text-[color:var(--dxy-ink)]/88 " +
          ...
          <StaggerGrid className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {fitBullets.map((bullet, i) => (
              <div
                key={bullet}
                className={
                  "dxy-card dxy-stagger-item flex items-start gap-3 p-5 dxy-fade-up-" +
                  Math.min(i + 1, 6)
                }
              >
                <svg
      ...
      <section id="outcomes" className="dxy-section">
        <div className="dxy-container">
          ...
          <div
            ref={outcomesRef}
            className="mt-10 dxy-stagger grid gap-4 md:grid-cols-2 lg:grid-cols-1"
          >
            {outcomes.map((item, i) => (
              <div
                key={item}
              ...
              >
                <div className="flex items-center gap-4 p-6 sm:p-7">
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[color:var(--dxy-border)] bg-[color:var(--dxy-paper)]">
                    <svg
                      className="h-5 w-5 text-[color:var(--dxy-sage)]"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={2}
      ...
      >
        <div className="dxy-container">
          ...
          <StaggerGrid className="mt-12 space-y-6">
            {services.map((service, i) => (
              <article
                key={service.title}
                className={
                  "dxy-card dxy-stagger-item dxy-card-interactive p-7 sm:p-10 dxy-fade-up-" +
                  Math.min(i + 1, 5)
                }
              >
      ...
      <section className="dxy-section">
        <div className="dxy-container">
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
            <article className="dxy-card dxy-card-interactive p-8 sm:p-10">
              <svg
                className="h-10 w-10 text-[color:var(--dxy-sage)]"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div className="space-y-6">
              <div className="dxy-card dxy-card-interactive p-7">
      ...
      >
        <div className="dxy-container">
          ...
          <div
            ref={stepsRef}
            className="relative mt-12 dxy-stagger grid gap-5 xl:grid-cols-4 md:grid-cols-2"
          >
            ...
            {steps.map((step, i) => (
              <article
                key={step.title}
                className={
                  "relative dxy-card dxy-stagger-item dxy-card-interactive p-7 sm:p-8 dxy-fade-up-" +
                  Math.min(i + 1, 4)
                }
              >
      ...
      >
        <div className="dxy-container">
          ...
          <div
            ref={whyRef}
            className="mt-10 dxy-stagger grid gap-4 md:grid-cols-2"
          >
            {whyDeltaxyBullets.map((bullet, i) => (
              <div
                key={bullet}
                className={
                  "dxy-card dxy-stagger-item dxy-card-interactive flex items-start gap-4 p-6 dxy-fade-up-" +
                  Math.min(i + 1, 6)
                }
              >
                <svg
                  className="mt-1 h-5 w-5 shrink-0 text-[color:var(--dxy-sage)]"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
      ...
      >
        <div className="dxy-container">
          <div className="mx-auto max-w-[880px] rounded-[28px] border border-[color:var(--dxy-border)] bg-[color:var(--dxy-card)] px-7 py-10 text-center shadow-[0_8px_24px_rgba(58,53,48,0.04)] sm:px-12 sm:py-14">
            <h2 className="font-serif text-[clamp(2.2rem,4.5vw,4.25rem)] leading-[0.99] text-[color:var(--dxy-ink)]">
